...
_AO='entity_id'
_AN='controller'
_AM='current_position'
_AL='stop_cover'
_AK='Eaton RFWC5'
_AJ='lutron_lip'
_AI='lutron_type'
_AH='color_temp_kelvin'
_AG='debug_leds'
_AF='number'
_AE='open_cover'
_AD='_sidebar_show'
_AC='/buttons_machine_panel.js'
_AB='license_key'
_AA='product'
_A9='area_name'
_A8='leap_button_number'
_A7='lower_button'
_A6='raise_button'
...
from._gate import _controller_entry,_global_license_key,_license_key_for_product,enforce_license
...
def _build_buttons_from_options(buttons_options):
	...
	for(L,A)in buttons_options.items():
		...
		if A.get(_P):B[_P]=_B
		H=A.get(_A3,{});I=A.get(_f,{});J=A.get(_g,{})
		if D is not _A or H or I or J:B[_h]={K:D or{},_A3:H,_f:I,_g:J}
		F.append(B)
	return F
_SCENE_GROUPS={}
import re as _re
...
async def _auto_refresh_button_layout(hass,entry):
	K=hass;A=entry;R=bool(A.data.get(_i));e=_R in A.data
	if R and e:return
	...
	if not B:return
	from.const import KEYPAD_LAYOUTS as S,KEYPAD_GENERIC as T;from.config_flow import _infer_keypad_type as h;U=A.data.get(_AI,'');V=h(U)if U else A.data.get(CONF_KEYPAD_TYPE,T);l,W=S.get(V,S[T])
	if R:
		for L in _iter_lutron_bridges(K):
			...
			for F in O.values():
				if B and str(F.get(_K,''))==B or G and str(F.get(_F,''))==G:I=F.get(_S,'')or'';K.config_entries.async_update_entry(A,data={**A.data,_R:I});_LOGGER.debug("Backfilled model_number=%r for '%s'",I,A.title);return
	...
	for L in _iter_lutron_bridges(K):
		...
		try:
			O=L.get_devices()
			for F in O.values():
				if B and str(F.get(_K,''))==B or G and str(F.get(_F,''))==G:I=F.get(_S,'')or'';break
		except Exception as P:_LOGGER.debug("get_devices() failed fetching model for '%s': %s",A.title,P)
		k={_i:H,_A5:d,_A6:C,_A7:D,_q:Z,_r:{},CONF_KEYPAD_TYPE:V,_R:I};_LOGGER.info("Auto-detected layout for '%s' (serial=%s): %d buttons, configurable=%s raise=%s lower=%s",A.title,B,len(H),d,C,D);K.config_entries.async_update_entry(A,data={**A.data,**k});return
...
async def _find_led_entities(hass,config_entry):
	...
	else:_LOGGER.warning("LED discovery for '%s': no LED entities mapped (keypad_type=%s). If your keypad has LEDs, configure led_entity manually in the options flow, or check the debug_leds service output.",B.title,B.data.get(_s))
	return G
...
async def _find_led_entities_by_button_entities(hass,config_entry):
	...
	if I is _A:return{}
	M=er.async_entries_for_device(Q,I.id);J=[A for A in M if A.domain==_j];G=[A for A in M if A.domain==_O and A.entity_id.endswith('_led')];_LOGGER.debug("LED discovery for '%s': device '%s' has %d button entities, %d LED switch entities",E.title,I.name,len(J),len(G))
	if not G:_LOGGER.debug("LED discovery: no switch.*_led entities on device '%s'",I.name);return{}
	R={A.entity_id for A in G};B={};K=E.data.get(_q,{})or{}
	for D in J:
...
async def _async_debug_leds(hass,call):
	C=hass;A=[];J=C.data.get(DOMAIN,{}).get(_t,{})
...
@websocket_api.websocket_command({vol.Required(_D):f"{DOMAIN}/save_keypad_config",vol.Required(_L):str,vol.Required(_T):dict})
@websocket_api.async_response
async def _ws_save_keypad_config(hass,connection,msg):
	E=connection;B=msg;A=hass;D=B[_L];F=B[_T];C=A.config_entries.async_get_entry(D)
	if C is _A or C.domain!=DOMAIN:E.send_error(B[_E],_l,f"Entry '{D}' not found");return
	A.config_entries.async_update_entry(C,options={**C.options,_T:F});A.async_create_task(A.config_entries.async_reload(D));E.send_result(B[_E],{_Z:_B})
...
async def _ws_discover_keypads(hass,connection,msg):
	...
	for L in _iter_lutron_bridges(C):
		...
		for B in M.values():
			...
			D.append({_K:F,_G:B.get(_G,''),I:B.get(_A9,''),_D:G,_s:J(G),_S:B.get(_S,'')or'',_F:str(B.get(_F,''))})
	from.config_flow import _discover_lip_keypads as O
	for H in await O(C):
		A=H[_V]
		if A[CONF_DEVICE_SERIAL]in E:continue
		D.append({_K:A[CONF_DEVICE_SERIAL],_G:A[_G],I:A.get(CONF_AREA_NAME,''),_D:A.get(_R,'')or _AJ,_s:A[CONF_KEYPAD_TYPE],_S:A.get(_R,''),_F:A[_F]})
	from.config_flow import _discover_rfwc5_keypads as P
	for H in P(C):
		A=H[_V]
		if A[CONF_DEVICE_SERIAL]in E:continue
		D.append({_K:A[CONF_DEVICE_SERIAL],_G:A[_G],I:A.get(CONF_AREA_NAME,''),_D:_AK,_s:A[CONF_KEYPAD_TYPE],_S:'RFWC5',_F:A[_F]})
...
async def _ws_add_keypad(hass,connection,msg):
	...
	if C.startswith('lip_'):
		...
		J={**I[_V],_G:H,CONF_DEVICE_NAME:H};A=await F.config_entries.flow.async_init(DOMAIN,context={_u:P},data=J)
	...
	for a in _iter_lutron_bridges(F):
		...
		for G in b.values():
			if str(G.get(_K,''))==C:M=G.get(_D,'');N=G.get(_A9,'');O=G.get(_G,'');T=G.get(_S,'')or'';L=str(G.get(_F,''))or L;break
		if M:break
	U=W(M);c=X(F,C,U,device_name=O,area_name=N,device_id=L);J={_G:H,CONF_DEVICE_SERIAL:C,CONF_DEVICE_NAME:O,CONF_AREA_NAME:N,CONF_KEYPAD_TYPE:U,_AI:M,_R:T,_F:L,**c};A=await F.config_entries.flow.async_init(DOMAIN,context={_u:P},data=J)
	if A.get(_D)==Q:E=A.get(R);D.send_result(B[_E],{_Z:_B,_L:E.entry_id if E else''})
...
async def _ws_license_status(hass,connection,msg):
	c='last_check';b='warn_only';a='max_version';Z='bound_instance';Y='binding';X='expires_at';W='lutron_keypad_controller';Q=connection;P='jti';O='lutron_leap';N='loaded';K='valid';J='has_key';G='error';A=hass;from.license import LicenseError as d,load_license_cache as e,validate_license_offline as f;D=_controller_entry(A)
	if D is _A:R=A.config_entries.async_entries(DOMAIN);D=R[0]if R else _A
	g=await async_get_instance_id(A);C={'instance_id':g,J:_C,K:_C,N:_C,G:_A}
	if D is _A:Q.send_result(msg[_E],C);return
	C[_L]=D.entry_id;C[N]=D.state.value==N;h=await async_get_integration(A,DOMAIN);i=str(h.version);S=await e(A);from.backends import get_backend as j;k={O:'Lutron Leap / Caséta',_AJ:'Lutron LIP','rfwc5_controller':_AK,W:'Lutron (legacy)'};E={}
	for T in A.config_entries.async_entries(DOMAIN):
		if T.data.get(_U):continue
		L=j(T);E[L.license_product]=getattr(L,'accepted_products',_A)or(L.license_product,)
	if not E:E[O]=O,W
	H=[]
	for I in sorted(E):
		l=E[I];F={_AA:I,'label':k.get(I,I),J:_C,K:_C,G:_A}
		for M in l:
			U=await _license_key_for_product(A,M)
			if not U:continue
			F[J]=_B
			try:
				B=f(U,current_version=i,expected_product=M);F.update({K:_B,'via_product':M,P:B.jti,X:B.expires_at,Y:B.binding,Z:B.instance_id,a:B.max_version,b:B.warn_only,G:_A})
				if S.get(P)==B.jti:F[c]=S.get('last_ok')
				break
			except d as m:F[G]=str(m)
		H.append(F)
	C['modules']=H
	if H:V=H[0];C.update({A:V[A]for A in(J,K,G,P,X,Y,Z,a,b,c)if A in V})
	Q.send_result(msg[_E],C)
@websocket_api.websocket_command({vol.Required(_D):f"{DOMAIN}/set_license",vol.Required(_L):str,vol.Required(_AB):str,vol.Optional(_AA):str})
@websocket_api.async_response
async def _ws_set_license(hass,connection,msg):
	K='license_keys';E=connection;C=msg;A=hass;from.license import EXPECTED_PRODUCT as F;B=_controller_entry(A)
	...
	G=(C.get(_AA)or F).strip();H=C[_AB].strip();D={**B.options};I={**(D.get(K)or{})};I[G]=H;D[K]=I
	if G==F:D[_AB]=H
	A.config_entries.async_update_entry(B,options=D)
	for J in A.config_entries.async_entries(DOMAIN):
		if not J.data.get(_U):A.async_create_task(A.config_entries.async_reload(J.entry_id))
...
def _update_sidebar_panel(hass):
	F='buttons-machine';E='_sidebar_shown';D='_panel_registered_url';C=hass;A=C.data.setdefault(DOMAIN,{});B=bool(A.get(_AD))
	if A.get(D)and A.get(E)==B:return
	if A.get(D):
		try:frontend.async_remove_panel(C,F)
		except Exception:pass
	try:frontend.async_register_built_in_panel(C,component_name='custom',sidebar_title=_a if B else _A,sidebar_icon='mdi:keyboard-outline'if B else _A,frontend_url_path=F,config={'_panel_custom':{_G:'buttons-machine-panel','module_url':_AC}},require_admin=_B);A[D]=_B;A[E]=B
	except Exception as G:_LOGGER.warning('Could not register Buttons Machine panel: %s',G)
...
async def _save_travel(hass):from homeassistant.helpers.storage import Store;B=Store(hass,1,_TRAVEL_STORE_KEY);A=hass.data.setdefault(DOMAIN,{}).get(_v)or{};await B.async_save({_b:float(A.get(_b)or _DEFAULT_TRAVEL_S),_X:dict(A.get(_X)or{})})
@callback
def _cover_travel_time(hass,cover_id):
	B=hass.data.get(DOMAIN,{}).get(_v)or{};A=(B.get(_X)or{}).get(cover_id)
...
def _discover_cover_cycle_covers(hass):
	...
	for A in hass.config_entries.async_entries(DOMAIN):
		if A.data.get(_U):continue
		B=A.options.get(_T)or A.data.get(_T)or{};F=B.values()if isinstance(B,dict)else B
		for C in F:
...
async def _calibrate_shade_travel(hass,cover_id):
	H='lutron_calibrate';B=cover_id;A=hass;from homeassistant.helpers.dispatcher import async_dispatcher_send as R;from homeassistant.components.persistent_notification import async_create as F;L=A.data.setdefault(DOMAIN,{});I=L.setdefault('_calibrating',set())
	if B in I:return
	I.add(B);D=5.;M=3.;N=_cover_travel_time(A,B)+6.;J=A.states.get(B);E=J.name if J is not _A and J.name else B
	async def G(service):await A.services.async_call(_w,service,{ATTR_ENTITY_ID:B},blocking=_B)
	F(A,f"Calibrating '{E}'. It will close, open briefly, then close again (~{int(N+D+M)}s) — please don't operate it meanwhile.",title=_a,notification_id=H)
	try:
		await G(_x);await asyncio.sleep(N);await G(_AE);await asyncio.sleep(D);await G(_AL);await asyncio.sleep(M);O=A.states.get(B);C=O.attributes.get(_AM)if O is not _A else _A
		if not C or float(C)<=0:F(A,f"Calibration of '{E}' failed — no movement detected (position={C}). Try again or set the travel time manually.",title=_a,notification_id=H);return
		C=min(float(C),1e2);P=D*1e2/C;K=int(round(min(max(P,3.),18e1)))+1;S=L.setdefault(_v,{_b:_DEFAULT_TRAVEL_S,_X:{}});S.setdefault(_X,{})[B]=float(K);await _save_travel(A);R(A,_TRAVEL_SIGNAL);_LOGGER.info("Calibrated '%s': opened to %.0f%% in %.1fs → full travel ≈ %.1fs → travel time set to %ds",E,C,D,P,K);F(A,f"Calibrated '{E}': reached {C:.0f}% in {int(D)}s → travel time set to {K}s.",title=_a,notification_id=H);await G(_x)
	except Exception as Q:_LOGGER.warning("Calibration of '%s' errored: %s",B,Q);F(A,f"Calibration of '{E}' errored: {Q}",title=_a,notification_id=H)
	finally:I.discard(B)
@callback
def _async_ensure_controller_entry(hass):
	A=hass
	if any(A.data.get(_U)for A in A.config_entries.async_entries(DOMAIN)):return
	A.async_create_task(A.config_entries.flow.async_init(DOMAIN,context={_u:_AN},data={}))
async def _async_setup_controller_entry(hass,entry):
	B=entry;A=hass;await _register_panel_once(A)
	if B.title in('Lutron Keypad Controller','Ctrlable Keypad Controller'):A.config_entries.async_update_entry(B,title=_a)
	A.data.setdefault(DOMAIN,{})[_v]=await _load_travel(A);await A.config_entries.async_forward_entry_setups(B,[_O,_AF,_j]);D=dr.async_get(A);C=D.async_get_device(identifiers={(DOMAIN,_AN)})
	if C is not _A:
		for E in list(C.config_entries):
			if E!=B.entry_id:D.async_update_device(C.id,remove_config_entry_id=E)
	return _B
@callback
async def async_setup(hass,config):
	...
	A.data[DOMAIN][_y]=C;A.services.async_register(DOMAIN,_AG,_async_debug_leds);return _B
async def async_setup_entry(hass,entry):
	B=hass;A=entry;B.data.setdefault(DOMAIN,{})
	if A.data.get(_U):return await _async_setup_controller_entry(B,A)
	E=B.data[DOMAIN].setdefault('_reload_listeners',set())
	if A.entry_id not in E:A.add_update_listener(_async_reload_entry);E.add(A.entry_id)
	await _register_panel_once(B);_async_ensure_controller_entry(B)
	if not await enforce_license(B,A):return _C
	B.data.setdefault(DOMAIN,{});B.data[DOMAIN].setdefault(_y,[]);B.data[DOMAIN].setdefault(_t,{});await _auto_refresh_button_layout(B,A)
	if not A.data.get(_i)or _R not in A.data:
		async def F(_event=_A):await _auto_refresh_button_layout(B,A)
		if B.state is CoreState.running:B.async_create_task(F())
		else:A.async_on_unload(B.bus.async_listen_once(EVENT_HOMEASSISTANT_STARTED,F))
	D=_build_buttons_from_options(A.options.get(_T,{}));G={_G:A.title,CONF_DEVICE_SERIAL:A.data.get(CONF_DEVICE_SERIAL,''),CONF_DEVICE_NAME:A.data.get(CONF_DEVICE_NAME,''),CONF_AREA_NAME:A.data.get(CONF_AREA_NAME,''),CONF_KEYPAD_TYPE:A.data.get(CONF_KEYPAD_TYPE,_A2),_F:A.data.get(_F,''),_i:A.data.get(_i,[]),_A5:A.data.get(_A5,[]),_A6:A.data.get(_A6),_A7:A.data.get(_A7),_q:A.data.get(_q,{}),_r:A.data.get(_r,{}),CONF_BUTTONS:D};C=LutronKeypadsController(B,G,config_entry=A)
	if D:C.async_register();_LOGGER.info("Keypad '%s' loaded from UI options with %d button(s)",A.title,len(D))
	...
	B.data[DOMAIN][_t][A.entry_id]=C;B.data[DOMAIN][_y].append(C)
	if not B.services.has_service(DOMAIN,_AG):B.services.async_register(DOMAIN,_AG,_async_debug_leds)
	await C.async_initialize();await _cleanup_orphaned_entities(B,A);await B.config_entries.async_forward_entry_setups(A,PLATFORMS);return _B
...
async def async_unload_entry(hass,entry):
	B=entry;A=hass
	if B.data.get(_U):return await A.config_entries.async_unload_platforms(B,[_O,_AF,_j])
	D=await A.config_entries.async_unload_platforms(B,PLATFORMS);C=A.data[DOMAIN].get(_t,{}).pop(B.entry_id,_A)
	if C is not _A:
		C.async_unregister()
...
def _normalize_led_map(raw_led_map,config_entry):
	D=config_entry;A=raw_led_map
	if not A:return A
	G=D.data;H=get_button_list(G.get(CONF_KEYPAD_TYPE,KEYPAD_GENERIC));I=sorted(A[_AF]for A in H if not A['is_raise']and not A['is_lower']);E=lambda k:isinstance(k,int)and k>100;B=sorted(A for A in A if E(A))
	if not B:return A
	C={A:B for(A,B)in A.items()if not E(A)};F=[A for A in I if A not in C]
	for(J,K)in zip(F,B):C[J]=A[K]
	_LOGGER.info("'%s': LED map: filled leftover LEAP IDs %s → buttons %s (final keys %s)",D.title,B,F[:len(B)],sorted(C));return C
class LutronKeypadsController:
	def __init__(A,hass,config,config_entry=_A):
		C=config_entry;B=config;A.hass=hass;A.name=B[_G];A.serial=str(B.get(CONF_DEVICE_SERIAL,'')).strip();A.device_id=str(B.get(_F,'')).strip();A.device_name=B.get(CONF_DEVICE_NAME,'').strip().lower();A.area_name=B.get(CONF_AREA_NAME,'').strip().lower();A.keypad_type=B.get(CONF_KEYPAD_TYPE,_A2);A.scene_group=B.get(_M,'').strip();A._config_entry=C;from.backends import get_backend as E;A._backend=E(C);A._buttons={}
		...
		if C is not _A:F=C.data.get(_r,{});A._leap_btn_map={int(A):B for(A,B)in F.items()}
	...
	def async_unregister(self):
		...
		if A._unsubscribe is not _A:A._unsubscribe();A._unsubscribe=_A;_LOGGER.debug("Buttons Machine '%s' unregistered",A.name)
	...
	def _clamp_cct(E,eid,target_cct):
		A=target_cct
		if A<=0:return A
		B=E.hass.states.get(eid)
		if B is _A:return A
		C=B.attributes.get('min_color_temp_kelvin');D=B.attributes.get('max_color_temp_kelvin')
		if C:A=max(A,int(C))
		if D:A=min(A,int(D))
		return A
	...
	@callback
	def _update_room_mode_led(self,btn_num,entities):B=btn_num;A=self;C=any((D:=A.hass.states.get(B))is not _A and D.state not in(_m,_n,_z,_W,_k)for B in entities);A._update_button_switch_state(B,C);A.hass.async_create_task(A._write_led_entity(B,C))
	@callback
	def _update_pathway_mode_led(self,btn_num,entities):C=entities;B=btn_num;A=self;D=bool(C)and all((E:=A.hass.states.get(B))is not _A and E.state not in(_m,_n,_z,_W,_k)for B in C);A._update_button_switch_state(B,D);A.hass.async_create_task(A._write_led_entity(B,D))
	@callback
	def _update_scene_mode_led(self,btn_num,entities):
		...
		def F(eid):
			C=eid;B=A.hass.states.get(C)
			if B is _A or B.state in(_m,_n,_z,_W,_k):return _C
			if not C.startswith(_N):return _B
			E=K.get(C,{});F=int(E.get(_H)or I);G=A._clamp_cct(C,int(E.get(_c)or J))
			if F>0:
				L=round((B.attributes.get(_H,0)or 0)/255*100)
				if abs(L-F)>5:return _C
			if G>0:
				D=B.attributes.get(_AH)
				if D is _A:
					H=B.attributes.get(_c)
					if H:D=round(1000000/H)
				if D is not _A and abs(int(D)-G)>100:return _C
			return _B
	...
	def _matches_event(A,event_data):
		...
		E=str(B.get('device_name','')).lower();F=str(B.get(_A9,'')).lower()
	...
	def _on_press(self,btn_num,btn_cfg):
		...
		K=C.get(CONF_ACTION_TYPE);M=I.get(_g,{});N=M.get(CONF_ACTION_TYPE,ACTION_NONE)!=ACTION_NONE and not C.get(_P,_C);O=K in A._HOLD_ACTIONS or C.get(_P,_C)or N
	...
	def _on_hold_event(self,btn_num):
		...
		if C is _A:return
		F=C.get(CONF_ACTION_TYPE);G=C.get(_P,_C);_LOGGER.debug("'%s': button %d hold event — action=%s cycle_dim=%s led_mode=%s (hold-to-dim is driven by cycle_dim, NOT led_mode)",A.name,B,F,G,C.get(CONF_LED_MODE));I=C.get(_h,{});H=I.get(_g,{})
		...
		if F==ACTION_RAISE:E=_o;D=A._get_last_ramp_lights()
		...
		try:
			while _B:
				...
				for B in entities:
					...
					if E is _A:continue
					if E.state==_m:
						if C==_o:await A.hass.services.async_call(_I,SERVICE_TURN_ON,{ATTR_ENTITY_ID:B,_e:1},blocking=_C);D=_C
						continue
					F=round((E.attributes.get(_H,0)or 0)/255*100);G=min(100,F+A._RAMP_STEP_PCT)if C==_o else max(0,F-A._RAMP_STEP_PCT)
					if G==F:continue
					D=_C
					if C==_d and G<=0:await A.hass.services.async_call(_I,SERVICE_TURN_OFF,{ATTR_ENTITY_ID:B},blocking=_C)
					else:await A.hass.services.async_call(_I,SERVICE_TURN_ON,{ATTR_ENTITY_ID:B,_e:G,_A0:A._RAMP_INTERVAL},blocking=_C)
				if D:break
	...
	def _next_ramp_dir(A,btn_num,entities=_A):
		...
		else:G=A._ramp_dirs.get(C,_d);B=_o if G==_d else _d
	...
	def _get_btn_light_entities(E,btn_cfg):
		...
		if B==ACTION_STATEFUL_SCENE:
			...
			if D:return[A for A in D.attributes.get(_AO,[])if A.startswith(_N)]
	...
	def _scene_light_entities(B,scene_id):
		...
		if A is _A:return[]
		return[A for A in A.attributes.get(_AO,[])if A.startswith(_N)]
	...
	async def _dispatch(A,btn_num,btn_cfg):
		...
		if F==ACTION_NONE:return
			...
			D=_normalize_targets(I);O=int(G.get(CONF_TARGET_BRIGHTNESS)or 0);P=int(G.get(CONF_TARGET_COLOR_TEMP)or 0);J=G.get(CONF_ENTITY_SETTINGS,{});Z=G.get(CONF_LED_MODE,LED_MODE_ROOM);a=G.get(_h,{});b=a.get(_A3,{});U=b.get('entity_settings',{})
			if Z==LED_MODE_SCENE:
				if A._is_btn_led_on(C):
					...
					for B in D:
						if B.startswith(_N):E=J.get(B,{});L=int(E.get(_H)or O);M=A._clamp_cct(B,int(E.get(_c)or P));Q=E.get(_p);N=float(E.get(W)or 0);R=float(E.get(X)or 0);_LOGGER.info("'%s': button %d  → %s bri=%d%% cct=%dK fade=%.1fs",A.name,C,B,L,M,N);await A._apply_light_settings(B,L,M,Q,N,R)
				...
				T=_C
				if D:Y=A.hass.states.get(D[0]);T=Y is not _A and Y.state not in(_m,_n,_z,_W,_k)
				c=any(int(J.get(A,{}).get(_H)or O)>0 or int(J.get(A,{}).get(_c)or P)>0 or bool(J.get(A,{}).get(_p))for A in D if A.startswith(_N))if D else _C
				if not T and c:
					for B in D:
						if B.startswith(_N):E=J.get(B,{});L=int(E.get(_H)or O);M=A._clamp_cct(B,int(E.get(_c)or P));Q=E.get(_p);N=float(E.get(W)or 0);R=float(E.get(X)or 0);await A._apply_light_settings(B,L,M,Q,N,R)
						else:H=B.split(_Y)[0];await A.hass.services.async_call(H,SERVICE_TURN_ON,{ATTR_ENTITY_ID:B},blocking=_B)
			...
			for B in D:
				if B.startswith(_N):E=J.get(B,{});L=int(E.get(_H)or O);M=int(E.get(_c)or P);Q=E.get(_p);N=float(E.get(W)or 0);R=float(E.get(X)or 0);await A._apply_light_settings(B,L,M,Q,N,R)
				else:H=B.split(_Y)[0];await A.hass.services.async_call(H,SERVICE_TURN_ON,{ATTR_ENTITY_ID:B},blocking=_B)
	...
	async def _activate_stateful_scene(A,btn_num,btn_cfg,scene_id):
		...
		await A._sync_leds(B);await A._write_group_leds(B,D);A._last_action={_D:ACTION_STATEFUL_SCENE,_A1:C,_j:B};_LOGGER.debug("Stateful scene '%s' activated on btn %d",C,B)
	...
	async def _cover_cycle(B,btn_num,targets):
		...
		if not C:return
		H=B.hass.states.get(C[0]);L=H.state if H is not _A else _A;I=H.attributes.get(_AM)if H is not _A else _A;M=asyncio.get_event_loop().time();A=B._cover_cycle_mem.get(G);J=M-A.get(N,0)if A else 1e9;U=A is not _A and J<=B._COVER_CYCLE_WINDOW;V=_cover_travel_time(B.hass,C[0]);Q=bool(A and A.get(O)==T and J<V)
		if Q:K=_AL;B._cover_cycle_mem[G]={O:'idle',E:A[E],N:M}
		else:
			...
			K=_AE if D==F else _x;B._cover_cycle_mem[G]={O:T,E:D,N:M}
		await B.hass.services.async_call(_w,K,{ATTR_ENTITY_ID:C},blocking=_B);B._last_action={_D:ACTION_COVER_CYCLE,_J:C,'state':K};_LOGGER.debug("'%s': button %d cover cycle — state=%s pos=%s since=%.1fs prev=%s moving=%s → %s",B.name,G,L,I,J if J<1e8 else-1,A,Q,K)
	...
	async def _raise(A,params):
		...
		if F==ACTION_COVER_CYCLE or _entities_are_covers(B):
			await A.hass.services.async_call(_w,_AE,{ATTR_ENTITY_ID:B},blocking=_B)
	...
	async def _lower(A,params):
		...
		D=A._last_action;F=D.get(_D);B=D.get(_J,[])
		if F==ACTION_COVER_CYCLE or _entities_are_covers(B):
			await A.hass.services.async_call(_w,_x,{ATTR_ENTITY_ID:B},blocking=_B)
			for(G,C)in A._buttons.items():
				if C.get(CONF_ACTION_TYPE)==ACTION_COVER_CYCLE and C.get(CONF_ACTION_TARGET):
					H=_normalize_targets(C[CONF_ACTION_TARGET])
