...
PLATFORMS=['sensor',_S,'select','text']
...
async def _auto_refresh_button_layout(hass,entry):
	...
	for L in _iter_lutron_bridges(K):
		...
		k={_W:H,_v:d,_w:C,_x:D,_y:Z,_k:{},CONF_KEYPAD_TYPE:V,_X:I};_LOGGER.info("Auto-detected layout for '%s' (serial=%s): %d buttons, configurable=%s raise=%s lower=%s",A.title,B,len(H),d,C,D);K.config_entries.async_update_entry(A,data={**A.data,**k});return
...
async def _find_led_entities_by_button_entities(hass,config_entry):
	...
	if H is _A:return{}
	K=er.async_entries_for_device(N,H.id);I=[A for A in K if A.domain==_A0];F=[A for A in K if A.domain==_S and A.entity_id.endswith('_led')];_LOGGER.debug("LED discovery for '%s': device '%s' has %d button entities, %d LED switch entities",D.title,H.name,len(I),len(F))
...
async def _async_debug_leds(hass,call):
	...
	for D in C.states.async_all(_S):
	...
	for E in Q.entities.values():
		if E.domain==_S and E.platform==_Q:A.append(f"  {E.entity_id}  unique_id={E.unique_id}  device_id={E.device_id}")
...
async def _ws_save_keypad_config(hass,connection,msg):
	...
	A.config_entries.async_update_entry(C,options={**C.options,_Z:F});A.async_create_task(A.config_entries.async_reload(D));E.send_result(B[_F],{_m:_B})
...
async def _ws_license_status(hass,connection,msg):
	...
	A[_M]=D.entry_id;A[H]=D.state.value==H;G=(D.options.get(_O)or'').strip();A[L]=bool(G)
...
@websocket_api.websocket_command({vol.Required(_D):f"{DOMAIN}/set_license",vol.Required(_M):str,vol.Required(_O):str})
@websocket_api.async_response
async def _ws_set_license(hass,connection,msg):
	...
	E={**C.options,_O:B[_O].strip()};A.config_entries.async_update_entry(C,options=E)
...
async def async_setup(hass,config):
	...
	A.data[DOMAIN][_n]=C;A.services.async_register(DOMAIN,_A5,_async_debug_leds);return _B
async def async_setup_entry(hass,entry):
	...
	await _register_panel_once(B);D=(A.options.get(_O)or'').strip()
	if not D:
		for O in B.config_entries.async_entries(DOMAIN):
			I=(O.options.get(_O)or'').strip()
			if I:D=I;break
	...
	if not(A.options.get(_O)or'').strip():B.config_entries.async_update_entry(A,options={**A.options,_O:D})
	...
	await remember_license_key(B,D);B.data.setdefault(DOMAIN,{});B.data[DOMAIN].setdefault(_n,[]);B.data[DOMAIN].setdefault(_l,{});await _auto_refresh_button_layout(B,A)
	...
	G=_build_buttons_from_options(A.options.get(_Z,{}));S={_K:A.title,CONF_DEVICE_SERIAL:A.data.get(CONF_DEVICE_SERIAL,''),CONF_DEVICE_NAME:A.data.get(CONF_DEVICE_NAME,''),CONF_AREA_NAME:A.data.get(CONF_AREA_NAME,''),CONF_KEYPAD_TYPE:A.data.get(CONF_KEYPAD_TYPE,_s),_E:A.data.get(_E,''),_W:A.data.get(_W,[]),_v:A.data.get(_v,[]),_w:A.data.get(_w),_x:A.data.get(_x),_y:A.data.get(_y,{}),_k:A.data.get(_k,{}),CONF_BUTTONS:G};F=LutronKeypadsController(B,S,config_entry=A)
...
async def _cleanup_orphaned_entities(hass,entry):
	...
	for A in er.async_entries_for_config_entry(B,D.entry_id):
		...
		elif C==f"{D.entry_id}_show_in_sidebar":B.async_remove(A.entity_id);_LOGGER.info('Removed old per-keypad sidebar switch: %s',A.entity_id)
async def async_unload_entry(hass,entry):
	D='_controller_switch';B=entry;A=hass;E=await A.config_entries.async_unload_platforms(B,PLATFORMS);C=A.data[DOMAIN].get(_l,{}).pop(B.entry_id,_A)
	if C is not _A:
		C.async_unregister()
		try:A.data[DOMAIN][_n].remove(C)
		except ValueError:pass
	if A.data.get(DOMAIN,{}).get(D)==B.entry_id:A.data[DOMAIN].pop(D,_A)
	return E
...
class LutronKeypadsController:
	...
	@callback
	def _update_room_mode_led(self,btn_num,entities):B=btn_num;A=self;C=any((D:=A.hass.states.get(B))is not _A and D.state not in(_f,_o,_p,_d,_c)for B in entities);A._update_button_switch_state(B,C);A.hass.async_create_task(A._write_led_entity(B,C))
	@callback
	def _update_pathway_mode_led(self,btn_num,entities):C=entities;B=btn_num;A=self;D=bool(C)and all((E:=A.hass.states.get(B))is not _A and E.state not in(_f,_o,_p,_d,_c)for B in C);A._update_button_switch_state(B,D);A.hass.async_create_task(A._write_led_entity(B,D))
	@callback
	def _update_scene_mode_led(self,btn_num,entities):
		...
		def F(eid):
			...
			if B is _A or B.state in(_f,_o,_p,_d,_c):return _C
	...
	async def _write_led_entity(A,btn_num,is_on):
		...
		try:await A.hass.services.async_call(_S,F,{ATTR_ENTITY_ID:D},blocking=_B);_LOGGER.debug("'%s': button %d LED '%s' → %s",A.name,B,D,'ON'if C else'OFF')
	...
	def _on_press(self,btn_num,btn_cfg):
		...
		K=C.get(CONF_ACTION_TYPE);M=I.get(_i,{});N=M.get(CONF_ACTION_TYPE,ACTION_NONE)!=ACTION_NONE and not C.get(_P,_C);O=K in A._HOLD_ACTIONS or C.get(_P,_C)or N
	...
	def _on_hold_event(self,btn_num):
		...
		if C is _A:return
		F=C.get(CONF_ACTION_TYPE);H=C.get(_P,_C);I=C.get(_j,{});G=I.get(_i,{})
		...
		try:
			while _B:
				...
				for B in entities:
					...
					else:await A.hass.services.async_call(_H,SERVICE_TURN_ON,{ATTR_ENTITY_ID:B,_V:G,_q:A._RAMP_INTERVAL},blocking=_C)
				if D:break
		...
		if F==ACTION_NONE:return
			...
			if Z==LED_MODE_SCENE:
				if A._is_btn_led_on(C):
					...
					for B in D:
						H=B.split(_R)[0];V=U.get(B,{});K=int(V.get(_G)or 0)
					...
					for B in D:
						...
						else:H=B.split(_R)[0];await A.hass.services.async_call(H,SERVICE_TURN_ON,{ATTR_ENTITY_ID:B},blocking=_B)
					A._last_action={_D:ACTION_ENTITY_TOGGLE,_I:D}
			else:
				T=_C
				if D:Y=A.hass.states.get(D[0]);T=Y is not _A and Y.state not in(_f,_o,_p,_d,_c)
	...
	async def _entity_toggle(A,targets):
		...
		for C in B:D=C.split(_R)[0];await A.hass.services.async_call(D,SERVICE_TOGGLE,{ATTR_ENTITY_ID:C},blocking=_B)
